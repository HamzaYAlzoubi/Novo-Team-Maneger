...
/* End Task Box*/
// timer
document.querySelector('.addTimer').addEventListener('click', function() {
    const timerList = document.getElementById('timer-list');
    const newTimer = document.createElement('div');
    newTimer.classList.add('new-timer');

    newTimer.innerHTML = `
        <span contenteditable="true" class="timer-input">00:00</span>
        <div class="timer-btns">
            <button class="add-time">Add</button>
            <button class="delete-time">Delete</button>
        </div>
    `;

    timerList.appendChild(newTimer);

    const timerInput = newTimer.querySelector('.timer-input');

    timerInput.addEventListener('input', function(e) {
        let value = timerInput.innerText.replace(/[^0-9]/g, '');
        value = value.padStart(4, '0');
        const minutes = value.slice(-4, -2);  // أخذ أول رقمين من اليمين
        const seconds = value.slice(-2);      // أخذ آخر رقمين
        timerInput.innerText = `${minutes}:${seconds}`;
    });

    newTimer.querySelector('.add-time').addEventListener('click', function() {
        const mainTimer = document.querySelector('#Ptimer');
        const mainMinutes = parseInt(mainTimer.children[0].innerText);
        const mainSeconds = parseInt(mainTimer.children[1].innerText);
        const [newMinutes, newSeconds] = timerInput.innerText.split(':').map(Number);

        let totalSeconds = mainMinutes * 60 + mainSeconds + newMinutes * 60 + newSeconds;
        const minutes = Math.floor(totalSeconds / 60);
        const seconds = totalSeconds % 60;

        mainTimer.children[0].innerText = String(minutes).padStart(2, '0');
        mainTimer.children[1].innerText = String(seconds).padStart(2, '0');
    });

    newTimer.querySelector('.delete-time').addEventListener('click', function() {
        const mainTimer = document.querySelector('#Ptimer');
        const mainMinutes = parseInt(mainTimer.children[0].innerText);
        const mainSeconds = parseInt(mainTimer.children[1].innerText);
        const [newMinutes, newSeconds] = timerInput.innerText.split(':').map(Number);
        ...
        timerList.removeChild(newTimer);
    });
});
